...
const PHOTO_VAULT_LEGACY_MIGRATION_PREFIX = 'aton-photo-vault-original-firestore-migrated-v2:';
...
async function deleteOriginalChunks(photoRef) {
  const snapshot = await photoRef.collection(PHOTO_VAULT_CHUNK_COLLECTION).get();
...
async function uploadOriginalChunks(photoRef, user, file, progressLabel = '') {
  ...
  await deleteOriginalChunks(photoRef);
...
async function getOriginalPhotoBlob(record) {
  if (!record?.key || !record.originalStored) {
    throw new Error('이 사진에는 공유된 원본이 없습니다.');
  }
  ...
  if (!chunks.length || (record.originalChunkCount && chunks.length !== Number(record.originalChunkCount))) {
...
async function downloadSelectedPhotoVaultItems(type) {
  ...
  if (selectedType === 'original') {
    const missing = selected.filter(record => !record.originalStored);
    if (missing.length) {
      alert(`선택한 사진 중 ${missing.length}장은 원본이 저장되어 있지 않습니다. 원본이 있는 사진만 선택하거나 압축본을 내려받아 주세요.`);
      return;
    }
  }
...
function renderPhotoVaultList(records) {
  ...
  list.innerHTML = records.map((record, index) => {
    ...
    const originalDisabled = record.originalStored ? '' : 'disabled';
    const originalLabel = record.originalStored ? '원본 다운로드' : '원본 없음';
    ...
    return `<article class="photo-vault-item" data-photo-card-key="${esc(record.key)}">
      ...
      <div class="photo-vault-item-actions">
        <button class="p" type="button" data-photo-action="download-original" data-photo-key="${esc(record.key)}" data-photo-index="${index}" ${originalDisabled}>${originalLabel}</button>

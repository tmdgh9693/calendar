function usesTripReportStatus(event) {
  return !!event && event.scope === '과';
}

function isTripReportCompleted(event) {
  if (!event) return false;
  if (event.workCompleted === true) return true;
  if (event.workCompleted === false) return false;
  return event.tripReportCompleted === true || event.sourceType === 'tripReport';
}
...
function setEventCompletionField(scope, completed = false, editable = true) {
  ...
  const isDeptEvent = scope === '과';

  if (row) row.classList.toggle('hidden', !isDeptEvent);
  if (input) {
    input.checked = isDeptEvent && !!completed;
    input.disabled = !isDeptEvent || !editable;
  }
}
...
async function saveEvent() {
  ...
  if (event.scope === '과') {
    ...
    const previousCompleted = previous ? isTripReportCompleted(previous) : false;
    ...
    if (completionChanged) {
      ...
      event.workCompletedSource = 'manual';
    } else if (previous) {
      [

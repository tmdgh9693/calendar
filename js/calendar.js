...
function canEditEvent(event) {
  if (!event) return false;
  if (typeof canWriteData === 'function' && !canWriteData()) return false;
  if (typeof isAdminUser === 'function' && isAdminUser() && event.scope === '과') return true;
  const me = ownerKey();
  return !!me && (
    event.ownerUid === me ||
...
function readEvent() {
  ...
  const eventId = $('evId').value || uid();
  const existing = data.events.find(item => item.id === eventId) || null;
  const originalOwnerUid = existing?.ownerUid || ownerKey();
  const originalCreatedByUid = existing?.createdByUid || originalOwnerUid || ownerKey();
  const originalSourceOwnerUid = existing?.sourceOwnerUid || (scope === '과' ? originalOwnerUid : '');

  const event = {
    id: eventId,
    scope,
    owner: scope === '개인' ? (existing?.owner || data.user) : '과',
    ownerUid: originalOwnerUid,
    createdByUid: originalCreatedByUid,
    sourceOwnerUid: originalSourceOwnerUid,
    visibleTo: Array.isArray(existing?.visibleTo)
      ? existing.visibleTo
      : (scope === '개인' ? [originalOwnerUid] : ['dept', originalOwnerUid]),
...
async function saveEvent() {
  if (typeof requireWriteAccess === 'function' && !requireWriteAccess('일정 저장')) return;
...
async function deleteEvent() {
  if (typeof requireWriteAccess === 'function' && !requireWriteAccess('일정 삭제')) return;
  const id = $('evId').value;

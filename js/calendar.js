...
function fillEvent(event) {
  ...
  $('evStartDate').value = event.startDate || event.date || today();
  $('evEndDate').value = event.endDate || event.startDate || event.date || today();
...
function readEvent() {
  ...
  return {
    ...
    date: $('evStartDate').value || today(),
    startDate: $('evStartDate').value || today(),
    endDate: $('evEndDate').value || $('evStartDate').value || today(),
    startH: start.h,
    startM: start.m,
    endH: end.h,
    endM: end.m,
...
async function saveEvent() {
  ...
  const startDateTime = new Date(`${event.startDate}T${String(event.startH).padStart(2, '0')}:${String(event.startM).padStart(2, '0')}:00`);
  const endDateTime = new Date(`${event.endDate}T${String(event.endH).padStart(2, '0')}:${String(event.endM).padStart(2, '0')}:00`);
  if (endDateTime < startDateTime) {
    alert('종료 일정은 시작 일정보다 빠를 수 없습니다.');
    return;
  }

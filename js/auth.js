...
async function applyNickname(user, preferredName = '') {
  ...
  // 로그인 화면에 입력한 이름을 가장 우선합니다.
  // 자동 로그인일 때는 Firestore에 저장된 이름을 씁니다.
  ...
  await ensureCloudUser(nickname);
...
async function login() {
  ...
  // 이미 로그인된 사용자가 이름만 새로 저장하는 경우입니다.
  if (auth?.currentUser) {
    try {
  ...
  // onAuthStateChanged에서도 처리되지만, 바로 화면에 이름이 반영되도록 한 번 더 적용합니다.
  try {
    await applyNickname(auth.currentUser, name);
    startRealtime();
    init();
...
function watchAuthState() {
  ...
  auth.onAuthStateChanged(async user => {
    if (user) {
      try {
        await applyNickname(user, pendingLoginName);
        startRealtime();
      } catch (error) {

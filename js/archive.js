function saveGenerated(type, sourceId) {
  if (typeof requireWriteAccess === 'function' && !requireWriteAccess('보관자료 저장')) return;
...
function canEditDoc(doc) {
  ...
  if (typeof isAdminUser === 'function' && isAdminUser() && doc?.scope === '과') return true;
  return (
    !doc.ownerUid ||
...
function editDoc(id) {
  if (typeof requireWriteAccess === 'function' && !requireWriteAccess('보관자료 수정')) return;
  const doc = data.docs.find(item => item.id === id);
...
async function deleteDoc(id) {
  if (typeof requireWriteAccess === 'function' && !requireWriteAccess('보관자료 삭제')) return;
  const doc = data.docs.find(item => item.id === id);

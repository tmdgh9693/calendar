function saveGenerated(type, sourceId) {
  ...
  const doc = {
    ...
    tripSnapshot: type === '출장복명서' ? {
      person: $('tPerson')?.value || '', rank: $('tRank')?.value || '', place: $('tPlace')?.value || '',
      date: $('tDate')?.value || '', endDate: $('tEndDate')?.value || '', reportDate: $('tReportDate')?.value || '',
      startH: $('tStartH')?.value || 9, startM: $('tStartM')?.value || 0, endH: $('tEndH')?.value || 18, endM: $('tEndM')?.value || 0,
      purpose: $('tPurpose')?.value || '', body: $('tBody')?.value || '', plan: $('tPlan')?.value || '',
      photos: Array.isArray(window.photos) ? window.photos : (typeof photos !== 'undefined' && Array.isArray(photos) ? photos : [])
    } : null
  };
...
async function withTripSnapshot(snapshot, callback) {
  ...
  try {
    Object.entries(mapping).forEach(([id,key]) => { if ($(id)) $(id).value = snapshot[key] ?? ''; });
    if (typeof photos !== 'undefined') photos = Array.isArray(snapshot.photos) ? snapshot.photos : [];
...
/*
 * HTML/DOC 내보내기 기능은 사용자 요청에 따라 비활성화했습니다.
 * 필요하면 아래 주석을 해제하여 다시 사용할 수 있습니다.

...
function applyMeetingFormLayout(sectionXml, sourceId, options = {}) {
  ...
  const department = options.department || report?.querySelector('h1')?.textContent?.trim() || $('mDept')?.value?.trim() || '항행정보시설과';
  ...
  const customHeadings = Array.isArray(options.headings) ? options.headings : [];
  hwpxSetParagraphText(doc, firstHeading, customHeadings[0] || (type === 'monthly' ? '󰊱 지난달 주요 성과' : '󰊱 지난주 주요 성과'));
  hwpxSetParagraphText(doc, secondHeading, customHeadings[1] || (type === 'monthly' ? '󰊲 이번 달 주요 계획' : '󰊲 이번주 주요 계획'));

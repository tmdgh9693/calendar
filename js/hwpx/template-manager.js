...
const DEFAULT_HWPX_TEMPLATE_FILENAMES = {
  meeting: 'meeting_weekly_template.hwpx',
  trip: 'trip_template.hwpx'
};
...
function applySelectedHwpxTemplate() {
  if (typeof isAdminUser === 'function' && !isAdminUser()) { alert('관리자만 공용 HWPX 템플릿을 변경할 수 있습니다.'); return; }
...
async function saveHwpxTemplatesToCloud() {
  ...
  if (typeof isAdminUser === 'function' && !isAdminUser()) {
    hwpxCloudStatus = '관리자만 공용 템플릿을 서버에 저장할 수 있습니다.';
    return false;
  }
...
async function addHwpxTemplateFromFile(file, options = {}) {
  if (typeof isAdminUser === 'function' && !isAdminUser()) { alert('관리자만 HWPX 템플릿을 등록할 수 있습니다.'); return false; }
  if (!file) {
...
function registerSelectedHwpxTemplate() {
  if (typeof isAdminUser === 'function' && !isAdminUser()) { alert('관리자만 HWPX 템플릿을 등록할 수 있습니다.'); return; }
  const file = $('hwpxTemplateFile')?.files?.[0] || null;
  return addHwpxTemplateFromFile(file);
}

async function downloadBundledHwpxTemplate(kind, meetingType = 'weekly') {
  try {
    await ensureBundledHwpxTemplates();
    const isTrip = kind === 'trip';
    const key = isTrip
      ? 'trip'
      : (meetingType === 'monthly' ? 'meetingMonthly' : 'meetingWeekly');
    const filename = isTrip
      ? '출장복명서_기본양식.hwpx'
      : (meetingType === 'monthly'
        ? '월간회의자료_기본양식.hwpx'
        : '주간회의자료_기본양식.hwpx');
    const buffer = bundledTemplateBuffer(key);
    const blob = new Blob([buffer], { type: 'application/vnd.hancom.hwpx' });
    ...
    console.error(error);
    alert('기본 양식을 내려받지 못했습니다: ' + (error.message || error));
  }
}
...
async function registerBundledHwpxTemplate() {
  ...
  try {
    await ensureBundledHwpxTemplates();
    const filename = DEFAULT_HWPX_TEMPLATE_FILENAMES[kind];
    const templates = normalizeHwpxTemplates();
    ...
    if (kind === 'trip' && window.BUNDLED_HWPX_TEMPLATES?.trip) {
      ...
      throw new Error('내장 기본 초안 데이터를 찾지 못했습니다.');
    }
...
async function clearHwpxTemplate() {
  if (typeof isAdminUser === 'function' && !isAdminUser()) { alert('관리자만 HWPX 템플릿을 삭제할 수 있습니다.'); return; }
  const kind = selectedHwpxKind();
  const templates = normalizeHwpxTemplates();
  const selected = selectedHwpxTemplate(kind);

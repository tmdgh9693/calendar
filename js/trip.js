...
function addTripPerson(rank = '', name = '') {
  ...
  row.innerHTML = `
    ...
    <div>
      <label>직급</label>
      <input class="trip-person-rank" value="${esc(rank || '')}" placeholder="직급">
    </div>
    <div>
      <label>성명</label>
      <input class="trip-person-name" value="${esc(name || '')}" placeholder="성명">
    </div>
    <button class="d" type="button" onclick="this.closest('.trip-person-row').remove(); renderTripPeople();">삭제</button>
  `;
          ...
          <div class="photo-grid sheet-grid">
            ${Array.from({ length: 6 }).map((_, index) => {
              ...
              return `<div class="photo-card">
                ${photo ? `<img src="${photo.data}" alt="출장 사진 ${number}">` : ''}
                <div class="cap">${photo ? esc(photo.cap || `사진 ${number}`) : '&nbsp;'}</div>
              </div>`;
            }).join('')}
          </div>
        </section>`)
...
function readTripCalendarEvent() {
...
async function saveTripToDeptCalendar() {
  const event = readTripCalendarEvent();

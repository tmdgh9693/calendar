...
function loadTrip() {
  ...
  if ($('tBody')) {
    $('tBody').value = [event.summary, event.result]
      .filter(Boolean)
      .map(text => '- ' + text)
      .join('\n');
  }
...
async function compressSmart(file) {
  ...
  while (result.bytes > targetBytes && maxWidth > 900) {
    maxWidth -= 160;
    quality = 0.58;
    result = await compress(file, maxWidth, quality);
  }
...
function tripPhotoSheetCount() {
  return photoChunks().length;
}

function tripAttachmentText() {
  // 사진이 없더라도 기관 양식의 사진대지 페이지와 붙임 표기는 유지합니다.
  return '붙임  사진대지 1부. 끝.';
}
    ...
    .map(
      (chunk, pageIndex) => `
        <section class="photo-page">
          <h1 class="trip-title">사진대지${pageIndex ? ` (${pageIndex + 1})` : ''}</h1>
          <div class="photo-grid">
            ${chunk
              .map(
                (photo, index) => `<div class="photo-card">
                  <img src="${photo.data}" alt="출장 사진 ${pageIndex * 4 + index + 1}">
                  <div class="cap">${esc(photo.cap || `사진 ${pageIndex * 4 + index + 1}`)}</div>
                </div>`
              )
              .join('')}
          </div>
...
function tripHwpxFilename() {
  ...
  return safeTitle
    ? `${datePrefix}_출장복명서(${safeTitle})_최종.hwpx`
    : `${datePrefix}_출장복명서_최종.hwpx`;
}

function makeTrip() {
  const date = $('tDate')?.value || today();
  const reportDate = $('tReportDate')?.value || date;
  const person = $('tPerson')?.value.trim() || data.user || '';
  const rank = $('tRank')?.value.trim() || '해양수산주사';

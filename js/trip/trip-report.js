...
function readTripCalendarEvent(photoCopies = []) {
  ...
  return {
    ...
    tripReportCompletedAt: new Date().toISOString(),
    tripReportDate: $('tReportDate')?.value || today(),
    tripReportOwnerUid: ownerKey(),
    tripReportOwner: data.user || '',
...
async function saveTripToDeptCalendar() {
  if (typeof requireWriteAccess === 'function' && !requireWriteAccess('과 캘린더 반영')) return;
  const photoCopies = await calendarPhotoCopies();
  const reportEvent = readTripCalendarEvent(photoCopies);

  if (!currentTripCalendarEventId) {
    currentTripCalendarEventId = findExistingTripCalendarEventId(reportEvent);
  }

  const existing = (data.events || []).find(item =>
    item && item.scope === '과' && item.id === currentTripCalendarEventId
  );
  ...
  const event = existing
    ? {
        ...
        workCompleted: true,
        workCompletedAt: existing.workCompletedAt || completedAt,
        workCompletedUpdatedAt: completedAt,
        workCompletedByUid: ownerKey(),
        workCompletedBy: data.user || '',
        workCompletedSource: 'tripReport',
        tripReportDate: $('tReportDate')?.value || today(),
        tripReportOwnerUid: ownerKey(),
        tripReportOwner: data.user || '',
        tripReportTitle: tripCalendarTitle(),
        tripReportPurpose: $('tPurpose')?.value.trim() || '',
        tripReportResult: $('tBody')?.value.trim() || '',
        tripReportPlan: $('tPlan')?.value.trim() || '',
        tripPhotos: Array.isArray(photoCopies) ? photoCopies : [],
    ...
    : {
        ...reportEvent,
        tripReportCompleted: true,
        tripReportCompletedAt: completedAt,
        tripReportUpdatedAt: completedAt,
        workCompleted: true,
  ...
  if (USE_FIREBASE) {
    try {
      ...
      alert('출장복명서는 생성됐지만 과 캘린더 완료 표시에는 실패했습니다.\n' + error.message);
      return;
    }
  }
}
...
async function makeTrip(options = {}) {
  ...
  const reflectToDept = await askYesNo('작성한 출장복명을 완료 처리하여 과 캘린더에 표시하시겠습니까?', '출장복명 완료 표시');

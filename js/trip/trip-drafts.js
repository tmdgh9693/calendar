...
function canSyncTripDraftsToCloud() {
  return !!(
    ...
    typeof USE_FIREBASE !== 'undefined' &&
    USE_FIREBASE && db && auth?.currentUser &&
    (typeof canWriteData !== 'function' || canWriteData())
  );
}
...
async function saveNamedTripDraft() {
  if (typeof requireWriteAccess === 'function' && !requireWriteAccess('출장복명 임시저장')) return;
...
async function deleteSelectedTripDraft() {
  if (typeof requireWriteAccess === 'function' && !requireWriteAccess('출장복명 임시파일 삭제')) return;
  const id = $('tripDraftSelect')?.value || '';
  if (!id) {

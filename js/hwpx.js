...
function hwpxCropPhotoForSlot(dataUrl) {
  // 사진대지 칸은 가로형(약 4:3)입니다.
  // 세로 사진도 빈 여백 없이 채우기 위해 중앙 기준으로 잘라 4:3으로 맞춥니다.
  return new Promise((resolve, reject) => {
    const image = new Image();
    image.onload = () => {
      ...
      if (sourceRatio > targetRatio) {
        // 가로로 긴 사진: 좌우를 조금 잘라 칸에 맞춤
        sw = sourceHeight * targetRatio;
        sx = (sourceWidth - sw) / 2;
      } else if (sourceRatio < targetRatio) {
        // 세로로 긴 사진: 위아래를 조금 잘라 칸에 맞춤
      ...
      // HWPX에 넣기 좋은 크기의 JPEG로 다시 저장합니다.
...
async function buildTripPhotoAssets() {
  ...
  // 실제 사진 칸 크기(셀 안쪽 여백 제외)에 맞춘 HWPUNIT 값입니다.
...
async function downloadHwpx(sourceId, filename, kind) {
  try {
    ...
    // 사진 파일의 BinData ID를 먼저 배정한 뒤 본문 사진칸에 연결합니다.

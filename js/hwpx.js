...
function applyTripFormLayout(sectionXml, map, assets) {
  ...
  hwpxSetCellText(doc, hwpxCell(attachment, 0, 1), chunks.length > 1 ? '사진대지 1' : '사진대지');
  ...
  for (let pageIndex = 1; pageIndex < chunks.length; pageIndex++) {
    ...
    hwpxSetCellText(doc, hwpxCell(attachmentCloneTable, 0, 1), `사진대지 ${pageIndex + 1}`);
    hwpxSetCellText(doc, hwpxCell(attachmentCloneTable, 0, 2), '끝.');

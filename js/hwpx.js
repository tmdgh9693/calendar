...
function hwpxEnsureEmptyPhotoBorderFill(header) {
  ...
  // 실제 한글 프로그램에서는 backSlash가 좌상단 → 우하단(\) 방향으로 표시됩니다.
  // slash는 반대 방향(/)이므로 끄고, backSlash 한 줄만 사용합니다.
  if (slash) {
...
async function downloadHwpx(sourceId, filename, kind, options = {}) {
  try {
    ...
    if (kind === 'meeting') {
      ...
      // 회의자료는 사용자가 보내준 내장 양식을 사용하므로 네트워크 fetch가 필요하지 않습니다.
      zip = await loadBundledMeetingTemplateZip(meetingType);

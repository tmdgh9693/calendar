...
function localSave() {
  const secureCloudOnly = window.SECURE_CLOUD_ONLY === true;
  localStorage.setItem(
    KEY,
    JSON.stringify({
      users: secureCloudOnly ? [] : data.users,
      user: secureCloudOnly ? '' : data.user,
      uid: secureCloudOnly ? '' : data.uid,
      userColors: data.userColors,
      userRanks: data.userRanks,
      events: secureCloudOnly ? [] : data.events,
      docs: secureCloudOnly ? [] : data.docs,
      hwpxTemplates: data.hwpxTemplates,
      hwpxTemplateSelections: data.hwpxTemplateSelections,
      deletedEventIds: secureCloudOnly ? [] : data.deletedEventIds,
      deletedDocIds: secureCloudOnly ? [] : data.deletedDocIds,
...
const VALID_TABS = new Set(['personal', 'dept', 'monthlySchedule', 'meeting', 'trip', 'archive', 'settings', 'photoVault']);
...
function tab(id, button, options = {}) {
  ...
  if (targetId === 'photoVault' && typeof renderPhotoVault === 'function') renderPhotoVault();
...
function askYesNo(message, title = '확인') {
  return new Promise(resolve => {
    const modal = document.getElementById('decisionModal');
    const titleEl = document.getElementById('decisionTitle');
    const messageEl = document.getElementById('decisionMessage');
    const yes = document.getElementById('decisionYesBtn');
    const no = document.getElementById('decisionNoBtn');
    if (!modal || !yes || !no) {

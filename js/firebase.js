...
async function initializeFirebase() {
  ...
  firebaseInitPromise = (async () => {
    ...
    console.warn('Firebase 초기화 실패. 로컬 저장 모드로 계속합니다.', error);
    USE_FIREBASE = false;
    auth = null;
    db = null;
    return false;
...
async function ensureCloudUser(name, preferredColor = '', preferredRank = '') {
  ...
  try {
    ...
    const cloudRank = String(userSnapshot.data()?.rank || '').trim();
    if (cloudRank) { data.userRanks[userId] = cloudRank; if (name) data.userRanks[name] = cloudRank; }
  } catch (error) {
  ...
  const rank = String(preferredRank || data.userRanks[userId] || data.userRanks[name] || '').trim();
...
function startRealtime() {
  ...
  snapshot.docs.forEach(doc => {
    ...
    if (user.uid && user.rank) { data.userRanks[user.uid] = user.rank; if (user.name) data.userRanks[user.name] = user.rank; }
